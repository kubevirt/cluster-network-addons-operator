...
func getCNA(data *templateData) {
	...
	err = refactorCrdString(writer.String(), &writer)
	check(err)
...
// perform manual changes on CRD manifest not generated by operator-sdk gen crd
func refactorCrdString(crdString string, writer io.Writer) error {
	crd := &extv1.CustomResourceDefinition{}
	err := yaml.Unmarshal([]byte(crdString), crd)
	if err != nil {
		return errors.Wrap(err, "failed parsing generated crd")
	}

	jsonBytes, err := json.Marshal(crd)
	if err != nil {
		return errors.Wrapf(err, "failed marshaling generated crd")
	}

	patchJSON := []byte(`[
		{"op": "add", "path": "/preserveUnknownFields", "value": "false"}
	]`)
	patch, err := jsonpatch.DecodePatch(patchJSON)
	check(err)

	modified, err := patch.Apply(jsonBytes)
	check(err)

	yamlBytes, err := yaml.JSONToYAML(modified)
	if err != nil {
		return errors.Wrapf(err, "failed JSONToYAML modified crd")
	}
	...
	_, err = writer.Write([]byte("---\n"))
	if err != nil {
		return errors.Wrapf(err, "failed adding yaml delimeter to  modified crd")
	}

	_, err = writer.Write(yamlBytes)
	if err != nil {
		return errors.Wrapf(err, "failed writing modified crd to string")
	}

	return nil

	//preserveUnknownFieldsPatch := jsonpatch.Operation{
	//	//Operation: "add",
	//	//Path:      "/preserveUnknownFields",
	//	//Value:     "false",
	//}
	//patches = append(patches, preserveUnknownFieldsPatch)
	//
	//for versionIdx, _ := range crd.Spec.Versions {
	//	lastHeartbeatTimePatch := jsonpatch.Operation{
	//		Operation: "add",
	//		Path:      fmt.Sprintf("/spec/versions/%d/schema/openAPIV3Schema/properties/status/properties/conditions/items/properties/lastHeartbeatTime/nullable", versionIdx),
	//		Value:     "true",
	//	}
	//	lastTransitionTimePatch := jsonpatch.Operation{
	//		Operation: "add",
	//		Path:      fmt.Sprintf("/spec/versions/%d/schema/openAPIV3Schema/properties/status/properties/conditions/items/properties/lastTransitionTimePatch/nullable", versionIdx),
	//		Value:     "true",
	//	}
	//
	//	patches = append(patches, lastHeartbeatTimePatch, lastTransitionTimePatch)
	//}
	//
	//jsonBytes = jsonpatch.NewPatch()
