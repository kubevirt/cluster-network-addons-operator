...
manager: $(GO)
	CGO_ENABLED=0 GOOS=linux $(GO) build -buildvcs=false -o $(BIN_DIR)/$@ ./cmd/...

manifest-templator: $(GO)
	CGO_ENABLED=0 GOOS=linux $(GO) build -buildvcs=false -o $(BIN_DIR)/$@ ./tools/manifest-templator/...
