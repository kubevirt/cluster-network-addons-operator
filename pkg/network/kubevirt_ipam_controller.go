...
func RenderKubevirtIPAMController(conf *cnao.NetworkAddonsConfigSpec, manifestDir string, clusterInfo *ClusterInfo) ([]*unstructured.Unstructured, error) {
	...
	// OCPTLSProfileCiphersToGoCipherNames return no result for TLS 1.3 cipher suites,
	// which is expected because kubevirt-ipam-controller doesn't allow configuring TLS 1.3 cipher suites.
