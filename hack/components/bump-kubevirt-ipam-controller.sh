...
{{ end }}' Service_kubevirt-ipam-claims-webhook-service.yaml
  ...
  cat Namespace_kubevirt-ipam-claims-system.yaml \
      ...
      Certificate_kubevirt-ipam-claims-serving-cert.yaml \
      Issuer_kubevirt-ipam-claims-selfsigned-issuer.yaml \
      MutatingWebhookConfiguration_kubevirt-ipam-claims-mutating-webhook-configuration.yaml > 002-kubevirtipamcontroller.yaml
...
# Cert Manager
echo "{{ if not .IsOpenshift }}" > data/kubevirt-ipam-controller/001-cert-manager.yaml
cert_mananger_manifest="https://github.com/cert-manager/cert-manager/releases/download/${CERT_MANAGER_VERSION}/cert-manager.yaml"
curl -L $cert_mananger_manifest >> data/kubevirt-ipam-controller/001-cert-manager.yaml
echo "{{ end }}" >> data/kubevirt-ipam-controller/001-cert-manager.yaml
sed -i '0,/name: cert-manager/{s/name: cert-manager/name: {{ .Namespace }}/}' data/kubevirt-ipam-controller/001-cert-manager.yaml
sed -i 's/namespace: cert-manager/namespace: {{ .Namespace }}/g' data/kubevirt-ipam-controller/001-cert-manager.yaml
sed -i 's/kube-system/{{ .Namespace }}/g' data/kubevirt-ipam-controller/001-cert-manager.yaml
sed -i 's|"cert-manager/|"{{ .Namespace }}/|g' data/kubevirt-ipam-controller/001-cert-manager.yaml
sed -i 's/[[:space:]]*$//' data/kubevirt-ipam-controller/001-cert-manager.yaml

# Kube Ipam controller
cp ${KUBEVIRT_IPAM_CONTROLLER_PATH}/config/cnao/002-kubevirtipamcontroller.yaml data/kubevirt-ipam-controller
sed -i '/app\.kubernetes\.io\//d' data/kubevirt-ipam-controller/002-kubevirtipamcontroller.yaml
...
sed -i 's|\(quay.io/jetstack/cert-manager[^:]*:\)[^"]*|\1'"$CERT_MANAGER_VERSION"'|g' test/releases/${CNAO_VERSION}.go
